...
## Scene 5 — Slides + script → video generation

**Build (rebuilt 2026-07-22 — replaces the old avatar/fighter picker entirely, see "Reorder built —
Speaker/Referent flow" above):**
- ✅ `VideoGen.tsx` beat 0: 3 processed slides (left, labeled "Speaker: Daniel") + generated script
  (right) + a big green blinking **"Generate Video Now"** button (`.fg-blink`).
- ✅ Beat 1: preview (`VideoPlaceholder`) + **"Submit Video"** button.
- ✅ Beat 2: **"Honorarium payout successful — €1,800"** feedback, then advances to `Publish.tsx`.
- ❌ **Now-obsolete assets, no longer needed**: the Street-Fighter character-select portraits for
  Sehouli/Prelog/Paula Cramer (that step doesn't exist in this scene anymore — those 3 are still used
  as photos in `SpeakerSelect.tsx`, just not here) and the "Effect of Pizza on the Human Body" locked
  video topic (video topic is now dynamic, driven by whichever topic Pharma picked in Shop).

**Assets needed:**
- Placeholder avatar-speaking preview asset (image or short clip) for the video, since the real
  AI-avatar pipeline output may not be ready to demo live.
- Per the 2026-07-22 follow-up call: an outfit/style picker for Daniel's avatar (framed as "which car
  do you want to drive today?") is a separate still-open item, not part of this rebuild — see "Open
  items from the 2026-07-22 follow-up call" below.
